...
const twoLettersFromEmail = (email: string) => {
	...
	return `${firstPart!.slice(0, 1).toUpperCase()}${secondPart
		.slice(0, 1)
		.toUpperCase()}`;
};
	...
	return (
		<div>
			...
			<ul className="space-y-2">
				{emails.map((info) => (
					<li key={info.email}>
						<Card className="flex flex-row items-center gap-2 bg-white p-3">
...
const RemoveInvitedEmailButton = ({
	email,
	token,
	...
	const deleteInvitationMutation =
		api.workspace.invitation.delete.useMutation();
