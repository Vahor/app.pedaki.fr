...
import { IconInfoCircleFill, IconLink, IconSchool, IconSpinner } from '@pedaki/design/ui/icons';
...
export const SubscriptionForm = ({ locale }: { locale: string }) => {
  ...
  function onSubmit(values: SubscriptionFormValues) {
    ...
    return wrapWithLoading(() => wait(createReservationMutation.mutateAsync(finalValues), 500), {
      loadingProps: {
        ...
        <FormField
          ...
          render={({ field }) => (
            ...
            >
              ...
              Aller au paiement ({priceMap[field.value ? 'yearly' : 'monthly']}€ par{' '}
              {field.value ? 'an' : 'mois'})
            </Button>
          )}
        />
