...
import {
	NotYourWorkspaceError,
	WorkspaceNotFoundError,
} from "@pedaki/models/errors/index.js";
import {
...
export const workspaceDataRouter = router({
	updateCurrentStatus: workspaceProcedure
		...
		.output(z.boolean())
		.meta({
			openapi: { method: "POST", path: "/workspace/{workspaceId}/status" },
		})
	...
	updateSettings: workspaceProcedure
		...
		.output(z.boolean())
		.meta({
			openapi: { method: "POST", path: "/workspace/{workspaceId}/settings" },
		})
		.mutation(async ({ input, ctx }) => {
			// The param is only here to respect the REST API
			// But we don't need it as we already have the workspaceId in the context
	...
	getSettings: workspaceProcedure
		...
		.meta({
			openapi: { method: "GET", path: "/workspace/{workspaceId}/settings" },
		})
		.query(async ({ input, ctx }) => {
			...
			return {
				...settings,
				defaultLanguage:
					settings.defaultLanguage as WorkspaceProperties["defaultLanguage"],
			};
		}),
});
