...
export const stripeRouter = router({
				...
				case "checkout.session.completed":
					{
						...
						const data = CheckoutSessionCompletedSchema.parse(
							event.data.object,
						);
						...
						// Create workspace
						const subscription = await stripeService.getSubscriptionInfo(
							data.subscription,
						);
						const pendingData = JSON.parse(pending.data) as z.infer<
				...
				case "checkout.session.expired":
					{
						...
						logger.info(
							"Deleted pending workspace creation after session expired",
							pendingId,
						);
					}
					break;
	...
	cancelCheckoutSession: publicProcedure
		...
		.mutation(async ({ input }) => {
			...
			await stripeService.expireCheckoutSession({
				sessionId: pending.stripePaymentId,
			});
		}),
	...
	getLatestSubscriptionData: workspaceProcedure
		...
		.meta({
			openapi: {
				method: "GET",
				path: "/stripe/{subdomain}/subscription/{type}/latest",
			},
		})
	...
	getCustomerPortalUrl: workspaceProcedure
		...
		.meta({
			openapi: {
				method: "GET",
				path: "/stripe/{subdomain}/customer-portal-url",
			},
		})
		.query(async ({ input, ctx }) => {
