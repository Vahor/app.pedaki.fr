...
import {Select, SelectContent, SelectItem, SelectTrigger, SelectValue} from '@pedaki/design/ui/select.js';
...
const Schema = CreateWorkspaceInput.and(z.object({cgu: z.boolean().refine(v => v, {message: 'Vous devez accepter les CGU'})}));
                            ...
                            <FormItem>
                                ...
                                <FormControl>
                                    <div className="relative">
                                        ...
                                        />
                                        <div
                                            className="absolute inset-y-0 right-0 px-3 border flex items-center pointer-events-none bg-secondary rounded-r-lg text-sm">
                                            <span className="text-gray-500 sm:text-sm">.pedaki.fr</span>
                                        </div>
                                    </div>
                                </FormControl>
                                <FormDescription>
                                    Sous domaine de votre workspace
                                </FormDescription>
                                <FormMessage/>
                            </FormItem>
                        )}
                                    ...
                                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                                        ...
                                        <SelectContent>
                                            {Object.entries(RegionMap[form.getValues().provider]).map(([region, label]) => (
                                                <SelectItem value={region} key={region}>{label}</SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                                </FormControl>
                                <FormDescription>
                                    The region where your workspace will be hosted
                                </FormDescription>
                            </FormItem>
                        )}
                    />
