...
export function getTransformer(options: GetTransformerOptions, matcher: tsPaths.MatchPath) {
  ...
  return (context: ts.TransformationContext): ts.Transformer<any> => {
    return function visitNode(node: any) {
      try {
        if (
          tsLib.isImportDeclaration(node) ||
          (tsLib.isExportDeclaration(node) && node.moduleSpecifier)
        ) {
          const importPath = node.moduleSpecifier && (node.moduleSpecifier as any).text;
          ...
          if (tsLib.isImportDeclaration(node)) {
            const newNode = tsLib.factory.updateImportDeclaration(
              node,
              node.modifiers,
              node.importClause,
              newModuleSpecifier,
            ...
            const newNode = tsLib.factory.updateExportDeclaration(
              node,
              node.modifiers,
              node.isTypeOnly,
              node.exportClause,
