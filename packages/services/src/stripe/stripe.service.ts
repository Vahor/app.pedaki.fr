...
class StripeService {
	...
	async createPayment({
		...
		const session = await this.stripe.checkout.sessions.create({
			...
			expires_at:
				Math.floor(Date.now() / 1000) + 60 * EXPIRES_AT_DURATION_MINUTES,
			...
			success_url: `${
				env.STORE_URL
			}/new/pending?token=${this.generatePendingJWT(
				{
...
export { stripeService, type Stripe };
