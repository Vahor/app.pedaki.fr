...
export class AwsServerProvider implements StackProvider<"aws"> {
	public async create(params: StackParameters<"aws">): Promise<StackOutputs> {
		...
		return tracer.startActiveSpan(
			...
			async (span) => {
				...
				await Promise.all(
					Object.entries(tags).map(([key, value]) => stack.setTag(key, value)),
				);

				const upRes = await stack.up();

				// Pulumi transform the array into an object {0: {value: ...}, 1: {value: ...}, ...}
				const formattedOutputs = Object.values(upRes.outputs).reduce(
					(acc, output) => {
	...
	private program =
		(params: StackParameters<"aws">) => async (): Promise<StackOutputsLike> => {
			const tags = this.tags(params);
			...
			const dbPassword = new random.RandomPassword(
				`${params.workspace.id}-db-password`,
				{
					length: 32,
					special: true,
					overrideSpecial: "_",
			...
			const passwordSalt = new random.RandomPassword(
			...
			const authSecret = new key.EncryptionKey(
				`${params.workspace.id}-auth-secret`,
			);
			const apiSecret = new key.EncryptionKey(
				`${params.workspace.id}-api-secret`,
			);
