...
const MaintenanceWindowSchema = z.string().refine(
	(value) => {
		...
		const [startDay, startHour, startMinute, endDay, endHour, endMinute] =
			value.split(/:|-/, 6);
		if (
			!startDay ||
			!startHour ||
		...
		const startHourNumber = Number.parseInt(startHour, 10);
		const startMinuteNumber = Number.parseInt(startMinute, 10);
		const endHourNumber = Number.parseInt(endHour, 10);
		const endMinuteNumber = Number.parseInt(endMinute, 10);
		...
		if (startDayIndex === endDayIndex) {
			...
			return (
				...
				(startHourNumber === endHourNumber &&
					startMinuteNumber < endMinuteNumber)
			);
		}
	...
	{
		message:
			'Value must be in the format "DAY:HH:MM-DAY:HH:MM" (e.g. "MON:00:00-TUE:00:00")',
	},
);
