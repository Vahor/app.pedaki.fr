...
const removeOldPendingWorkspaceCreations = async () => {
	...
	profiler.done({
		message: `[pending] clear-old-data deleted ${count} pending workspaces`,
	});
};
...
const removeOldWorkspaceTokens = async () => {
	...
	const moreThanOneToken = Object.values(groupedByWorkspaceId).filter(
...
const removeOldWorkspaceInvitations = async () => {
	...
	const count = result.count;
	logger.info(
		`[invitations] clear-old-data deleted ${count} workspace invitations`,
	);
};
