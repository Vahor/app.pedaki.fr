...
const main = async () => {
	...
	const response =
		await workspaceService.getLatestSubscription(WORKSPACE_SUBDOMAIN);
	...
	if (response) {
		...
		if (env.DELETE_OLD_STACK) {
			logger.info(
				`Deleting previous stack for subscription ${previousSubscriptionId}`,
			);
	...
	try {
		await invitationService.addPendingInvite(
			workspaceId,
			"developers@pedaki.fr",
		);
	} catch (error) {
		logger.error("Error while adding pending invite", error);
	}

	await resourceService.upsertStack(
		stackParameters(workspaceId, subscriptionId, authToken),
	);
